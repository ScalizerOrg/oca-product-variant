...
class TestProductConfiguratorAttribute(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_template_attribute_value = cls.env["product.template.attribute.value"]
        cls.product_configuration_attribute = cls.env["product.configurator.attribute"]
        cls.product_template = cls.env["product.template"].with_context(check_variant_creation=True)
    ...
    def test_possible_value_ids_filtered_correctly(self):
        ...
        template = self.product_template.create({
            "name": "Test Product",
            "attribute_line_ids": [
                (0, 0, {"attribute_id": attr.id, "value_ids": [(6, 0, [value1.id])]})
            ],
        })
        conf_attr = self.product_configuration_attribute.create({
...
class TestTargetedProductCoverage(TransactionCase):
    ...
    def test_build_attributes_domain_dict_format_scenario(self):
        template = self.env["product.template"].create({
            ...
            "attribute_line_ids": [
                (0, 0, {"attribute_id": self.attribute.id, "value_ids": [(6, 0, [self.value.id])]})
            ],
        })

        dict_attrs = [{"attribute_id": self.attribute.id, "value_id": self.value.id}]
        domain, cont = self.env["product.product"]._build_attributes_domain(template, dict_attrs)
        ...
        domain, cont = self.env["product.product"]._build_attributes_domain(template, dict_attrs_no_value)
    ...
    def test_configuration_validity_creating_variants_scenario(self):
        ...
        product = self.env["product.product"].with_context(creating_variants=True).create({
            "name": "Validity Product", "product_tmpl_id": template.id
        })
